namespace :bets do
  ...
  task my_bets: :environment do
    ...
    my_bets = [ ['1. FSV Mainz 05', 'Hannover 96', 1.95, "Home", hamish, 10],
    ['AS Saint-Étienne', 'Amiens SC', 1.63, 'Home', hamish, 10],
    ["Hertha BSC",  "VfB Stuttgart",  2.09, "Home", hamish, 10],
    ["Crystal Palace FC",  "Huddersfield Town",  1.83, "Home", hamish, 10],
    ["Leicester City FC", "Brighton & Hove Albion", 1.77, "Home", hamish, 10],
    ["Borussia Dortmund", "Hertha BSC", 1.41, "Home", hamish, 10],
    ["Newcastle United FC", "Tottenham Hotspur FC", 1.78, "Away", hamish, 10],
    ["1. FC Köln",  "Hamburger SV", 2.05, "Home", hamish, 10],
    ["Olympique Lyonnais",  "RC Strasbourg Alsace", 1.33, "Home", hamish, 10],
    ["EA Guingamp", "RC Strasbourg Alsace", 1.78, "Home", hamish, 10],
    ["Valencia CF", "UD Las Palmas",  1.83, "Home", hamish, 10],
    ["FC Barcelona",  "Real Betis", 1.21, "Home", hamish, 10],
    ["FC Bayern München", "Bayer Leverkusen", 1.3,  "Home", hamish, 10],
    ["Benevento", "Bologna FC", 3.34, "Draw", hamish, 10],
    ["Levante UD",  "RC Deportivo La Coruna", 3.45, "Draw", hamish, 10],
    ["Athletic Club", "Getafe CF",  1.54, "Home", hamish, 10],
    ["Paris Saint-Germain", "Amiens SC",  1.09, "Home", hamish, 10],
    ["SPAL Ferrara",  "Udinese Calcio", 3.38, "Draw", hamish, 10],
    ["OGC Nice",  "EA Guingamp",  1.91, "Home", hamish, 10],
    ["Girona FC", "Málaga CF",  3.48, "Draw", hamish, 10],
    ["OGC Nice",  "ES Troyes AC", 1.52, "Home", hamish, 10],
    ["FC Augsburg", "Bor. Mönchengladbach", 3.56, "Draw", hamish, 10] ]

    my_bets.each do |match_array|
      create_bet(match_array)
    end
  end
end
